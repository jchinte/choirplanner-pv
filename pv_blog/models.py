...
import pv_blog.streams as blocks
...
class BlogPage(Page):
    ...
    body= StreamField([
        ("heading", wagtail_blocks.CharBlock(classname="full title", icon="title")),
    ...
    api_fields = [
        ...
        APIField('body'),
        APIField('image_card', serializer=ImageRenditionField('fill-300x300', source='image'))
    ]
...
class BlogAuthorsOrderable(Orderable):
    ...
    page = ParentalKey("pv_blog.BlogDetailPage", related_name="blog_authors")
...
class BlogChildPagesSerializer(Field):
    def to_representation(self, child_pages):
        ...
        for child in child_pages:
            ...
            if hasattr(child.specific, 'banner_image'):
                post_dict.update({'banner': ImageRenditionField('fill-800x800').to_representation(child.specific.banner_image)})
            if hasattr(child.specific, 'custom_title'):
                post_dict.update({'custom_title':child.specific.custom_title})
            if hasattr(child.specific, 'subtitle'):
                post_dict.update({'subtitle':child.specific.subtitle})
...
class BlogListingPage(RoutablePageMixin, Page):
    ...
    max_count = 1
    subpage_types = ['pv_blog.VideoBlogPage', 'pv_blog.ArticleBlogPage']
...
class BlogDetailPage(Page):
    ...
    subpage_types = []
    parent_page_types = ['pv_blog.BlogListingPage']
    ...
    categories = ParentalManyToManyField("pv_blog.BlogCategory", blank=True)
    #blog_authors = ParentalManyToManyField("pv_blog.BlogAuthor", blank=True)
    content = StreamField(
        [
            ...
            ("cta", blocks.CTABlock()),
            ("full_richtext", blocks.YouTubeVideoBlock2()),
        ],
        null=True,
        blank=True,
    )
...
class VideoBlogPage(BlogDetailPage):
    ...
    api_fields=BlogDetailPage.api_fields+[
